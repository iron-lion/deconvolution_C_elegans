"""
run_Eder_deconvolution.py
========================
Test all deconvolution algorithms on the real Eder et al. (2024)
single-worm RNA_seq dataset with multiple parameter configurations.

Reproduces the full pipeline from notebooks/deconvolution_for_pub.ipynb:
  - Same data loading (Eder_ref_data)
...
Usage (from the project root):
    python tests/run_Eder_deconvolution.py
"""
...
OUT_DIR  = os.path.join(ROOT, "results", "Eder")
print(OUT_DIR)
os.makedirs(OUT_DIR, exist_ok=True)

sys.path.insert(0, SRC_DIR)

from utils import Eder_ref_data, load_cell_to_group
...
def build_aligned_matrices(target_df, min_markerscore=0.08, min_marker=5):
    """
    Align the single worm RNA seq. matrix and the reference matrix to their
...
def save_results(
    ...
    raw_csv = os.path.join(OUT_DIR, f"Eder_{tag}_proportion.csv")
    ...
    plot_csv = os.path.join(OUT_DIR, f"Eder_{tag}_plot_data.csv")
    ...
    ax.set_title(f"Eder et al. swRNAseq — {tag}")
    ax.grid(axis="y", alpha=0.4)
    plt.tight_layout()

    svg_path = os.path.join(OUT_DIR, f"Eder_{tag}_proportion.svg")
...
# The notebook uses 0.08 as the production value for swRNAseq.
MIN_MARKERSCORE_GRID = [0.00, 0.04, 0.08]
...
# notebook ("for swRNAseq we use min_marker=0").
...
PARAM_GRID = [
    {
        ...
        "kwargs": {"param_C": 0.01,  "param_e": 0.1,  "max_workers": 8},
    },
    {
        "tag"   : "lsvr_C0.001_e0.1",
        "algo"  : "LSVR",
        "fn"    : run_multiprocess_deconvolution,
        "kwargs": {"param_C": 0.001, "param_e": 0.1,  "max_workers": 8},
    },
]
...
def main():
    ...
    # ── 1. Load Eder et al. data (same as notebook) ──────────────────────────
    print("Loading Eder et al. swRNAseq data …")
    os.chdir(os.path.join(ROOT, "notebooks"))   # utils.py uses relative paths
    target_df, time_points = Eder_ref_data()
    ...
    summary_csv = os.path.join(OUT_DIR, "Eder_algorithm_comparison.csv")
    ...
    #   • Eder_comparison_by_marker.svg     — fixes min_markerscore=0.08
    #   • Eder_comparison_by_markerscore.svg — fixes min_marker=15
    # Kept as the canonical Eder_algorithm_comparison.svg for backwards compat.
    ...
    _facet_bar_plot(
        ...
        title="Eder et al. swRNAseq — within-day agreement by min_marker (min_markerscore=0.08)",
        svg_path=os.path.join(OUT_DIR, "Eder_comparison_by_marker.svg"),
    )
    ...
    _facet_bar_plot(
        ...
        title="Eder et al. swRNAseq — within-day agreement by min_markerscore (min_marker=15)",
        svg_path=os.path.join(OUT_DIR, "Eder_comparison_by_markerscore.svg"),
    )

    # Keep canonical name pointing at the min_marker plot (backwards compat)
    import shutil
    shutil.copy(
        os.path.join(OUT_DIR, "Eder_comparison_by_marker.svg"),
        os.path.join(OUT_DIR, "Eder_algorithm_comparison.svg"),
    )
    print(f"Canonical copy: {os.path.join(OUT_DIR, 'Eder_algorithm_comparison.svg')}")
